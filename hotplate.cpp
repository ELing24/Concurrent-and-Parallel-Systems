...
class Task {
  ...
  int row;
  int start = id * row + 1;
  int end = id == ntasks - 1 ? N - 1 : start + row;
public:
  ...
  Task(int id, int ntasks, double* to, double* from, int N, int steps, int row)
    : id(id), ntasks(ntasks), to(to), from(from), N(N), steps(steps), row(row)
  {
  }
...
int main(int argc, char* argv[]) {
  ...
  //intialize the number of rows to assign to each thread
  int row = (N-2) / ntasks;
  ...
  for(int i = 0; i < ntasks; ++i)
  {
      ...
      tasks.emplace_back(i,ntasks, to,from,N,steps, row);
